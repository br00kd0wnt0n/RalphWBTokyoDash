    ...
    <section>
      ...
      {activeTab === 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="bg-bg-secondary border border-border rounded-card p-6">
            <h4 className="text-accent text-xs font-mono uppercase tracking-widest mb-3">Client Data</h4>
            <ul className="space-y-3 text-xs text-text-secondary">
              ...
              <li className="p-3 bg-bg-tertiary rounded-inner">
                ...
                <p>Client 2026 event schedule</p>
              </li>
            </ul>
          </div>
          <div className="bg-bg-secondary border border-border rounded-card p-6">
            <h4 className="text-accent text-xs font-mono uppercase tracking-widest mb-3">Industry Benchmarks</h4>
            <ul className="space-y-3 text-xs text-text-secondary">
              <li className="p-3 bg-bg-tertiary rounded-inner">
          ...
          <div className="bg-bg-secondary border border-border rounded-card p-6">
            <h4 className="text-accent text-xs font-mono uppercase tracking-widest mb-3">Ralph Methodology</h4>
            <ul className="space-y-3 text-xs text-text-secondary">
              <li className="p-3 bg-bg-tertiary rounded-inner">
      ...
      {activeTab === 1 && (
        <div className="space-y-4">
          <div className="bg-bg-secondary border border-border rounded-card p-6">
            <h4 className="text-accent text-xs font-mono uppercase tracking-widest mb-4">Base Forecast (Organic Only)</h4>
            <div className="font-mono text-sm leading-loose">
              <p className="text-accent mb-3">baseGrowth[platform] =</p>
              <div className="pl-6 flex items-center gap-3 flex-wrap">
                ...
                <div className="p-3 bg-bg-tertiary rounded-inner">
                  ...
                  <p className="text-text-dim text-xs mt-1">Monthly adjustment: National Holiday 1.20x, Nov peak 1.25x, Jan dip 0.85x</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-bg-secondary border border-border rounded-card p-6">
            <h4 className="text-accent text-xs font-mono uppercase tracking-widest mb-4">With Strategy</h4>
            <div className="font-mono text-sm leading-loose">
              <p className="text-accent mb-3">withStrategy[platform] = max( baseGrowth, organicBase x strategyMult x seasonal )</p>
          ...
          <div className="bg-bg-secondary border border-border rounded-card p-6">
            <h4 className="text-accent text-xs font-mono uppercase tracking-widest mb-4">With Campaigns</h4>
            <div className="font-mono text-sm leading-loose">
              <p className="text-accent mb-3">withCampaigns[platform] = max( baseGrowth, organicBase x strategyMult x (seasonal + campLift) )</p>
              <div className="pl-6 space-y-3">
                <div className="p-3 bg-bg-tertiary rounded-inner">
        ...
        <div className="bg-bg-secondary border border-border rounded-card p-6">
            ...
            <BarChart data={seasonalChartData} margin={{ top: 30, right: 20, left: 10, bottom: 0 }}>
              ...
              <Bar dataKey="factor" animationDuration={800}>
                {seasonalChartData.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={entry.factor >= 1.15 ? '#E91E8C' : entry.factor >= 1.0 ? '#9B1260' : '#5a5a72'}
                  />
                ))}
              </Bar>
            </BarChart>
          ...
          <div className="mt-4 p-4 bg-bg-tertiary rounded-inner">
            <p className="text-text-secondary text-xs leading-relaxed">
              Seasonal factors are derived from 2024-2025 historical patterns. Peak months (May for National Holiday, November for winter seasonals) show 20-25% uplift. January and February show 10-15% dip post-holiday.
            </p>
          </div>
        </div>
      )}
                      ...
                      {(Object.entries(s.followConversionMultiplier) as [Platform, number][]).map(([p, v]) => (
                        <div key={p} className="flex items-center gap-2">
                          ...
                          <div className="flex-1 h-2 bg-bg-tertiary rounded-full overflow-hidden">
                            <div className="h-full bg-accent rounded-full" style={{ width: `${(v / 1.5) * 100}%` }} />
                          </div>
