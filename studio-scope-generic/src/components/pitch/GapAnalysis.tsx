    ...
    if (audienceComposition.tourist < 15 && audienceComposition.core <= 50) {
      ...
      if (delta > 0) {
        recs.push({
          ...
          detail: `Tourist segment at ${audienceComposition.tourist}%. Tourists have the highest IG follow conversion (1.3x) and drive the social-to-attendance pipeline that differentiates the client's strategy.`,
          additionalFollowers: delta,
          newTotal: currentGrowth + delta,
          confidence: 'low',
        });
    ...
    <div className="mt-8">
      ...
      <div className={`flex items-center gap-3 mb-6 p-4 bg-bg-secondary border rounded-card ${hasFollowerGap ? 'border-border' : 'border-success/30'}`}>
        <div className="flex-1">
          ...
          <p className={`text-2xl font-semibold font-mono ${hasFollowerGap ? 'text-accent' : 'text-success'}`}>{formatGrowth(currentGrowth)}</p>
          {paidBudget > 0 && (
                    ...
                    <div className="w-full h-2.5 bg-bg-primary rounded-full overflow-hidden relative">
                      ...
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor: reachesTarget ? '#4ade80' : '#E91E8C',
                        }}
                      />
                    </div>
