    ...
    <section>
      <SectionHeader
        ...
        subtitle={compact
          ...
          : <>Configure audience, platform allocation, and campaigns. <span className="live-hint">The forecast below updates live.</span> <button onClick={() => document.getElementById('module-3')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="inline-flex items-center gap-1 ml-1 text-accent hover:text-accent-light transition-colors text-xs font-medium">Jump to Forecast <span aria-hidden="true">&darr;</span></button></>
        }
        id="module-2"
      />
