...
const MODULES = [
  {
    icon: BarChart3,
    label: 'State of Play',
    description: 'Historical performance across all platforms from June 2023 to December 2025. Where the client stands today.',
  },
  {
    icon: Sliders,
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-bg-secondary border border-border rounded-card p-8">
      ...
      <div className="flex items-center gap-3 mb-2">
        <div>
          <h1 className="font-display text-2xl font-semibold text-accent flex items-center gap-2">
            Studio Scope
      ...
      <p className="text-text-secondary text-sm leading-relaxed mb-6">
        Social Growth Forecasting
      </p>

      <div className="border-t border-border pt-6 mb-6">
        <h2 className="text-text-primary text-sm font-medium mb-2">How this dashboard works</h2>
        <p className="text-text-secondary text-xs leading-relaxed mb-4">
          Studio Scope is an interactive forecasting tool that projects social media growth across Instagram, X, TikTok, Facebook, and YouTube. Every chart responds to your inputs: adjust the audience mix, change platform content volume, toggle campaigns on and off, and watch the 12-month forecast update live.
        </p>
        <p className="text-text-secondary text-xs leading-relaxed">
          Start with the <span className="text-accent">Strategy Controls</span> to set your audience targeting and content allocation, then scroll to the <span className="text-accent">Forecast</span> to see the projected impact. The <span className="text-accent">Methodology</span> section documents every assumption and data source for full transparency.
        </p>
      </div>
      ...
      <button
        onClick={onClose}
        className="w-full mt-6 px-6 py-3 bg-accent text-bg-primary font-medium text-sm rounded-inner hover:bg-accent-light transition-colors"
      >
