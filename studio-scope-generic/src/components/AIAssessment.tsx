...
const RECOMMENDATIONS = [
  {
    ...
    impact: 'High',
    impactColor: '#4ade80',
    observation: 'The client currently publishes an average of 10 posts/month on TikTok, well below the platform optimum for entertainment venues. Early data (Jul-Aug 2023) showed explosive growth when content was posted, with 22K and 21K monthly gains respectively.',
  ...
  },
  {
    ...
    impact: 'High',
    impactColor: '#4ade80',
    observation: 'Current content is primarily single-language. With 42.4M inbound visitors projected for 2025 (and growing), the client is leaving significant international reach on the table, particularly on Instagram and YouTube where visitor audiences are concentrated.',
  ...
  },
  {
    ...
    impact: 'Medium',
    impactColor: '#fbbf24',
    observation: 'March-May (Spring Festival + National Holiday) and October-November (Fright Nights + Halloween + winter seasonals) show campaign overlap. Currently, these campaigns are treated independently, but coordinated stacking could amplify growth during these windows.',
    recommendation: 'Create unified content narratives that bridge overlapping campaigns. For example: "Fright Nights x Halloween" as a single content arc rather than two separate campaigns. This maximizes the campaign multiplier effect during high-traffic periods.',
    ...
    <section>
      ...
      {!revealed && (
        <div className="bg-bg-secondary border border-border rounded-card p-12 text-center">
          <Sparkles className="w-10 h-10 text-accent-dim mx-auto mb-4" />
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-2 px-6 py-3 bg-accent text-bg-primary font-medium text-sm rounded-inner hover:bg-accent-light transition-colors disabled:opacity-50"
          >
            {loading ? (
              <>
      ...
      {revealed && (
        <div className="space-y-4">
          <div className="bg-bg-secondary border border-accent/30 rounded-card p-6 mb-6">
            <div className="flex items-center gap-2 mb-3">
              <Sparkles className="w-4 h-4 text-accent" />
              <h3 className="text-accent text-sm font-medium">Assessment Summary</h3>
            </div>
            <p className="text-text-secondary text-sm leading-relaxed">
              Based on the current audience composition (Casual Expansion preset), platform allocation, and campaign calendar, the client has strong growth fundamentals but is underinvesting in its two highest-potential channels: TikTok and international audience reach. The following four recommendations, if implemented together, could add an estimated 25,000 to 40,000 additional followers beyond the base forecast.
            </p>
          </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2 p-3 bg-bg-tertiary rounded-inner">
                        ...
                        <p className="text-accent text-xs font-medium">{rec.projectedImpact}</p>
                      </div>
                    </div>
                  </div>
                </div>
